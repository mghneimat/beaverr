...
export async function loadDashboardBundle(t) {
  ...
  const currency = financials.currencyCode || 'CZK';
  const sinkingSync = syncSinkingFundStashes(
    migratedBudget || {},
    financials.sections,
    t,
    (amount) => formatCurrency(amount, currency),
  );
  const mergedBudget = sinkingSync.budget;
  if (budgetMigrated || sinkingSync.changed) {
    await setData('beaverr_budget', mergedBudget);
  }
  const financialsWithBudget = (budgetMigrated || sinkingSync.changed)
    ? { ...financials, budget: mergedBudget }
    : financials;

 ...
 * @param {DailyLog[]} logs
 * @param {string} [throughDate] - inclusive upper bound (YYYY-MM-DD); default through yesterday
 * @param {Date} [now]
 * @returns {string[]}
 */
export function missingDaysInCycle(cycle, logs, throughDate, now = new Date()) {
  const today = isoDateKey(now);
  const endIso = throughDate && throughDate <= today ? throughDate : undefined;
  const days = enumerateDaysFrom(cycle.startedAt, endIso, now);
...
export function isCycleBackfillPending(cycle, logs, now = new Date()) {
  ...
  return missingDaysInCycle(cycle, logs, undefined, now).length > 0;
}

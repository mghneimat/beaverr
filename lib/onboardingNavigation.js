...
async function persistHistory() {
  ...
  await patchOnboardingState({ navHistory: memoryHistory });
}
...
export function useOnboardingScreen(options = {}) {
  ...
  /** URL step wins over in-component progressStep to avoid stale type on back navigation. */
  const normalized = normalizeNavParams(searchParams) || {};
  /** @type {Record<string, string|undefined>} */
  const params = { ...normalized };
  if (!params.step && progressStep) {
    params.step = progressStep;

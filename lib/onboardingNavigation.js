...
import { pathnameToRouteName } from './onboardingProgress';
...
const ONBOARDING_BACK_FALLBACK = /** @type {Record<string, string>} */ ({
  '/(onboarding)/setup-mode': '/(onboarding)/welcome',
  '/(onboarding)/quick-housing': '/(onboarding)/income',
  '/(onboarding)/splash-household': '/(onboarding)/setup-mode',
  '/(onboarding)/household': '/(onboarding)/splash-household',
  '/(onboarding)/splash-residence': '/(onboarding)/splash-household',
  '/(onboarding)/citizenship': '/(onboarding)/splash-residence',
  '/(onboarding)/location': '/(onboarding)/splash-residence',
...
/**
 * @param {string|string[]|null|undefined} segments
 * @returns {string}
 */
export function segmentsToOnboardingRoute(segments) {
  const list = Array.isArray(segments) ? segments : [];
  const leaf = list.slice().reverse().find(
...
/**
 * Preferred on web — pathname is always a string; useSegments() can be non-array mid-transition.
 * @param {string|null|undefined} pathname
 * @returns {string}
 */
export function pathnameToOnboardingRoute(pathname) {
  const name = pathnameToRouteName(pathname);
...
export function getNavHistory() {
  return Array.isArray(memoryHistory) ? memoryHistory.slice() : [];
}
...
export function useOnboardingScreen(options = {}) {
  ...
  const route = pathnameToOnboardingRoute(pathname);
...
export function __resetNavHistoryForTests(entries = []) {
  memoryHistory = Array.isArray(entries) ? entries.slice() : [];

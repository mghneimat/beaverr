...
/**
 * @param {{ snapshot: object, triggered_rules: object[], locale: string, kb_chunk_ids?: string[] }} input
 * @returns {{ systemPrompt: string, userMessage: string, promptVersion: string, kbChunkIds: string[] }}
 */
export function buildLlmRequest({ snapshot, triggered_rules, locale, kb_chunk_ids = [] }) {
  const payload = {
    v: snapshot.v ?? 1,

...
export async function loadAccountRegistrationFields(userId) {
  ...
  const fromSplit = splitDisplayName(household.displayName);
  const explicitFirst = typeof settings.accountFirstName === 'string'
    ? settings.accountFirstName.trim()
    : '';
  const explicitLast = typeof settings.accountLastName === 'string'
    ? settings.accountLastName.trim()
    : '';
  const firstName = explicitFirst || fromSplit.firstName;
  const lastName = explicitLast || fromSplit.lastName;
...
export async function saveAccountRegistrationFields({
  ...
  await setData('beaverr_settings', {
    ...
    accountFirstName: trimmedFirst,
    accountLastName: trimmedLast,
  });

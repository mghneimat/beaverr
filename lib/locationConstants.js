...
export const PRE_ALPHA_CURRENCY_CODE = 'CZK';
 ...
 * @param {'en'|'cs'|string} [locale]
 * @returns {string}
 */
export function getCountryDisplayName(countryCode, locale = 'en') {
  const code = countryCode || PRE_ALPHA_COUNTRY_CODE;
  const country = COUNTRIES.find((c) => c.code === code);
  if (!country) return code;
  return locale === 'cs' && country.nameCs ? country.nameCs : country.name;
}

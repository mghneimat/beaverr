...
export function buildTrackerPreviews({
  ...
  const dayProgress = Math.max(
    1 / 24,
    Math.min((now.getHours() + now.getMinutes() / 60) / 24, 1),
  );
  const dailyPace = computeSpendingPace({
    spent: spentToday,
    budgetEnvelope: dailyBudget,
    elapsedUnits: dayProgress,
    totalUnits: 1,
    remaining: remainingToday,
  });
  ...
  return {
    ...
    daily: {
      ...
      paceLevel: dailyPace.level,
      paceColor: dailyPace.color,
      spendingPace: dailyPace,
    },
    weekly: {

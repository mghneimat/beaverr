 ...
 * @returns {import('../schema').Goal}
 */
export function buildManualDebtGoal(debt, index, t, endDate = null) {
  const debtId = resolveDebtId(debt, index);
  return {
    ...buildDebtGoalFromEntry({ ...debt, id: debtId }, index, t, endDate),
    autoCreated: false,
  };
}
 ...
 * @param {{ creditor: string, balance: number, minPayment?: number, endDate?: string|null }} fields
 * @param {(key: string, params?: object) => string} t
 */
export async function appendDebtPayoffGoal(fields, t) {
  const { endDate, ...debtFields } = fields;
  const debt = createDebtEntry(debtFields);
  const { index } = await appendDebtEntry(debt);
  const goal = buildManualDebtGoal(debt, index, t, endDate);

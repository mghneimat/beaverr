 ...
 * @returns {CustomStash|null}
 */
export function findAutoStashBySourceKey(budget, sourceKey) {
  if (!sourceKey) return null;
  return getCustomStashes(budget).find((stash) => stash.sinkingSourceKey === sourceKey) || null;
}
 ...
 * @returns {boolean}
 */
function autoSinkingStashMetaEqual(a, b) {
  return a.name === b.name
    && a.sinkingSourceKey === b.sinkingSourceKey
    && Number(a.sinkingTargetAmount) === Number(b.sinkingTargetAmount)
    && a.sinkingDueDate === b.sinkingDueDate
    && Number(a.sinkingSuggestedMonthly) === Number(b.sinkingSuggestedMonthly)
    && normalizeStashDescription(a.description) === normalizeStashDescription(b.description)
    && a.autoCreated === true;
}
 ...
 * @returns {string|null}
 */
function clampStashDescription(description) {
  const normalized = normalizeStashDescription(description);
  if (!normalized) return null;
  if (normalized.length <= STASH_DESCRIPTION_MAX_LENGTH) return normalized;
  return normalized.slice(0, STASH_DESCRIPTION_MAX_LENGTH - 1).trimEnd();
}
...
/**
 * Create or update an auto-managed sinking-fund stash keyed by source.
 * @param {Budget|null|undefined} budget
 * @param {{
 *   sourceKey: string,
 *   name: string,
 ...
 * }} candidate
 * @param {Date} [now]
 * @returns {{ budget: Budget, stash: CustomStash, created: boolean, changed: boolean }}
 */
export function upsertAutoSinkingStash(budget, candidate, now = new Date()) {
  const base = budget && typeof budget === 'object' ? { ...budget } : {};
  const stashes = [...getCustomStashes(base)];
  const existingIdx = stashes.findIndex((s) => s.sinkingSourceKey === candidate.sourceKey);
  const name = normalizeStashName(candidate.name).slice(0, STASH_NAME_MAX_LENGTH);
  const description = clampStashDescription(candidate.description);
  const meta = {
    autoCreated: true,
    sinkingSourceKey: candidate.sourceKey,
    sinkingTargetAmount: candidate.targetAmount,
    sinkingDueDate: candidate.dueDate,
    sinkingSuggestedMonthly: candidate.suggestedMonthly,
  };

  if (existingIdx >= 0) {
    const current = stashes[existingIdx];
    const updated = {
      ...current,
    ...
    if (autoSinkingStashMetaEqual(current, updated)) {
      return {
        budget: base,
        stash: current,
        created: false,
    ...
    stashes[existingIdx] = updated;
    return {
      budget: { ...base, customStashes: stashes },
      stash: updated,
      created: false,
      changed: true,
    };
  ...
  const stash = {
    id: `stash_sf_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`,
    name: name || candidate.sourceKey,
    balance: 0,
    createdAt: isoDateKey(now),
    ...meta,
    ...(description ? { description } : {}),
...
/**
 * Retire auto sinking-fund stashes whose source no longer exists.
 * @param {Budget|null|undefined} budget
 * @param {Set<string>} activeSourceKeys
 * @returns {{ budget: Budget, changed: boolean }}
 */
export function retireOrphanAutoSinkingStashes(budget, activeSourceKeys) {
  const base = budget && typeof budget === 'object' ? { ...budget } : {};
  const stashes = getCustomStashes(base);
  if (stashes.length === 0) return { budget: base, changed: false };

  let changed = false;
  const nextStashes = stashes.flatMap((stash) => {
    if (!stash.autoCreated || !stash.sinkingSourceKey) return [stash];
    if (activeSourceKeys.has(stash.sinkingSourceKey)) return [stash];

    const balance = Number(stash.balance) || 0;
    if (balance <= 0) {
      changed = true;
      return [];
    }
  ...
  if (!changed) return { budget: base, changed: false };
  return { budget: { ...base, customStashes: nextStashes }, changed: true };
}

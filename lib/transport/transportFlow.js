...
/**
 * Display label for a vehicle in sinking-fund and budget copy.
 * @param {Record<string, unknown>|null|undefined} vehicle
 * @param {number} index
 * @param {(key: string, params?: Record<string, string|number>) => string} t
 * @returns {string}
 */
export function getVehicleDisplayLabel(vehicle, index, t) {
  const customName = typeof vehicle?.displayName === 'string' ? vehicle.displayName.trim() : '';
  if (customName) return customName;
  const categoryKey = CATEGORY_LABELS[vehicle?.category] || vehicle?.category;
  const categoryLabel = t(categoryKey);
  return t('dashboard.savingsScreen.sinkingFund.vehicleLabel', {
...
export function buildTransportPayload(state) {
  ...
  return {
    ...
    vehicles: hasVehicle ? vehicles.map((v) => ({
      ...
      hasVignette: v.category === 'passenger' || v.category === 'motorcycle' ? v.hasVignette === true : false,
      vignetteAmount: v.hasVignette && v.vignetteAmount ? parseFloat(v.vignetteAmount) || 0 : null,
      vignetteValidUntil: v.hasVignette ? v.vignetteValidUntil || null : null,

...
/**
 * Status copy for spending pace notices and banners.
 * @param {Function} t
 * @param {{
 *   messageKey: string,
 ...
 * }} params
 */
export function buildSpendingPaceMessages(t, {
  messageKey,
  backfillNote = false,
}) {
  let lineMessage = t(`dashboard.spendingPace.${messageKey}`);

  if (backfillNote) {
    lineMessage = `${lineMessage} · ${t('dashboard.spendingPace.backfillPendingNote')}`;
  }

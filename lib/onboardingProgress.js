...
export function normalizeResumeRoute(resume) {
  ...
  const aliased = LEGACY_RESUME_ROUTE_ALIASES[resume] ?? resume;
  if (!aliased.startsWith('/(onboarding)/')) return null;
  return getKnownOnboardingResumeRoutes().has(aliased) ? aliased : null;
}

...
export function repairOnboardingState(state) {
  ...
  if (next.dashboardUnlocked === true && !next.setupMode) {
    const quickSteps = new Set(['quick-setup', 'quick-setup-done']);
    next.setupMode = quickSteps.has(next.currentStep) ? 'quick' : 'full';
    changed = true;
  }

  if (next.resumeRoute === '/(onboarding)/budget') {
    next.resumeRoute = '/(onboarding)/budget-setup';
    changed = true;
  }
...
export function shouldShowContinueQuestionnaire(state) {
  return shouldShowQuestionnaireBanners(state) && Boolean(getSavedResumeRoute(state));
}
...
export function shouldShowStartQuestionnaire(state) {
  return shouldShowQuestionnaireBanners(state) && !getSavedResumeRoute(state);
}
...
export function getQuestionnaireStartRoute(_state) {
  return QUESTIONNAIRE_WELCOME_ROUTE;
}
...
export function getResumeRoute(state) {
  return getSavedResumeRoute(state) || QUICK_RESUME_ROUTE;
}
...
export function getQuestionnaireNavigationRoute(state) {
  const saved = getSavedResumeRoute(state);
...
export const ONBOARDING_SECTIONS = /** @type {OnboardingSectionDef[]} */ ([
  {
    id: 'entry',
    routes: ['welcome', 'consent', 'setup-mode', 'quick-setup'],
    startPercent: 0,
    endPercent: 4,
  },
  {
  ...
  {
    id: 'location',
    routes: ['splash-location', 'location', 'citizenship', 'residence-permit', 'occupation'],
    startPercent: 10,
    endPercent: 22,

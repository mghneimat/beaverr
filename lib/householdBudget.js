...
export function aggregateHouseholdCosts(sections, t) {
  ...
  const safePets = asArray(pets);
  const safeSubs = asArray(subs);
  const safeOtherCosts = asArray(otherCosts);
  ...
  if (housing.type === 'renting') {
    ...
    if (housing.utilitiesMode === 'itemized' && asArray(housing.utilityItems).length) {
      asArray(housing.utilityItems).forEach((item) => {
  ...
  if (housing.hasOtherCosts && asArray(housing.otherCostRows).length) {
    asArray(housing.otherCostRows).forEach((r, idx) => {
      if (r.amount) housingItems.push({ label: r.label || `Other cost ${idx + 1}`, amount: r.amount, frequency: 'monthly' });
    });
  }
  if (housing.type === 'family' && housing.contributesToFamily === true && asArray(housing.familyContributionRows).length) {
    asArray(housing.familyContributionRows).forEach((r, idx) => {
  ...
  if (housing.govtTaxes) {
    ...
    if (asArray(gt.customItems).length) {
      asArray(gt.customItems).forEach((item, idx) => {
  ...
  if (transport.hasVehicle && asArray(transport.vehicles).length) {
    asArray(transport.vehicles).forEach((v, vi) => {
      const prefix = asArray(transport.vehicles).length > 1 ? `Vehicle ${vi + 1} ` : '';
  ...
  safeOtherCosts.forEach((c) => {
    if (c.amount) {
      ...
      const row = { label: costLabel, amount: parseFloat(c.amount), frequency: c.frequency || 'monthly' };
      if (c.name === 'groceries') {
        groceryItems.push(row);
      } else {
        otherItems.push(row);
      }
    }
  ...
  if (groceryItems.length > 0) {
    byCategory.push({ category: 'groceries', label: t('onboarding.budget.budgetSplit.cat.groceries'), items: groceryItems });
    allCosts.push(...groceryItems);
  }
...
export function loadRecurringCommitments(sections, debts, t) {
  ...
  if (transport.hasVehicle && asArray(transport.vehicles).length) {
    asArray(transport.vehicles).forEach((v, vi) => {
      ...
      if (!monthlyAmount || monthlyAmount <= 0) return;
      const prefix = asArray(transport.vehicles).length > 1
  ...
  asArray(debts).forEach((debt, idx) => {

...
/**
 * Dashboard deep link — open a tab and scroll/highlight a jar or section.
 * @param {import('expo-router').Router} router
 * @param {string} route
 * @param {string|null|undefined} focusJar
 * @param {string|null|undefined} [focusSection]
 */
export function navigateFromDashboardWithFocus(router, route, focusJar, focusSection) {
  setScreenTransitionDirection('forward');
  /** @type {Record<string, string>} */
  const params = {};
  if (focusJar) params.focusJar = focusJar;
  if (focusSection) params.focusSection = focusSection;
  if (Object.keys(params).length > 0) {
    router.push({ pathname: `/(app)/${route}`, params });
    return;
  }
  router.push(`/(app)/${route}`);
}

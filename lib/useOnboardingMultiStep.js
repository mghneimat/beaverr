...
export function useOnboardingMultiStep(routeName, options = {}) {
  ...
  const loadStepFromStorageRef = useRef(options.loadStepFromStorage);
  onFocusRef.current = options.onFocus;
  loadStepFromStorageRef.current = options.loadStepFromStorage;
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      (async () => {
        if (onFocusRef.current) {
          await onFocusRef.current();
        }
        if (cancelled || !loadStepFromStorageRef.current || !registry?.storageKey) return;

        const { getData } = await import('./storage');
        const saved = await getData(registry.storageKey);
        const loaded = loadStepFromStorageRef.current(saved);
    ...
    }, [urlStep, registry?.storageKey, setStep]),
  );

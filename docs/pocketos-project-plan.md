# 🏠 PocketOS — AI Coding Assistant Master Prompt (v2)
...
6. **Design system lock** — the design system defined in this file is the single source of truth. Never introduce new colours, fonts, spacing units, or component patterns that aren't in this file. If you believe an addition is needed, propose it and wait for confirmation.
...
**PocketOS** is a household financial clarity tool aimed at:
...
**Core insight:** Most budgeting apps track *past* spending. PocketOS helps you see **what today costs you**, how much free money you actually have *right now*, keeps you ahead of upcoming payments, and gives you a clear path toward your financial goals.
...
│  │  (Tailwind tokens, colour variables, typography scale)   │ │
│  └──────────────────────────────────────────────────────────┘ │
└────────────────────────────────┬────────────────────────────┘
                                 │ HTTPS (Phase 2+)
          ┌──────────────────────┼───────────────────────────┐
...
> **Lock rule:** This design system is the single source of truth and is frozen. No AI tool may introduce new colours, fonts, radius values, shadow styles, or spacing units outside of what is defined below. If a change is needed, propose it and wait for confirmation before implementing.

### UI Inspiration

The visual language is inspired by the Baserow onboarding pattern: clean white left-panel form areas, bold sans-serif headings, chunky rounded pill/chip selectors, a strong indigo CTA button, and generous whitespace. Calm, trustworthy, fintech-meets-productivity. Not a bank. Not a spreadsheet. On mobile the two-panel layout collapses to single-column.

### Colour Tokens

Defined in `tailwind.config.js` (as custom colours) and exported from `constants/colors.js`:

| Token | Light | Dark | Usage |
|---|---|---|---|
| `bg` | `#F7F8FA` | `#0F1724` | Page background |
| `surface` | `#FFFFFF` | `#172032` | Cards, form panels |
| `primary` | `#4F46E5` | `#6366F1` | CTA buttons, active states, links |
| `primary-hover` | `#4338CA` | `#818CF8` | Button hover |
| `positive` | `#22C55E` | `#22C55E` | Surplus, healthy numbers |
| `warning` | `#F59E0B` | `#F59E0B` | Near limit |
| `danger` | `#EF4444` | `#EF4444` | Deficit, overrun |
| `debt` | `#7C3AED` | `#9F6BFF` | All debt-related elements — always purple |
| `text` | `#111827` | `#E8EAF0` | Body text |
| `muted` | `#6B7280` | `#8B95A8` | Labels, helpers, captions |
| `border` | `#E5E7EB` | `#2A3650` | Input borders, dividers |
| `chip-active` | `#EEF2FF` | `#312E81` | Selected pill/chip background |
| `chip-active-border` | `#4F46E5` | `#6366F1` | Selected pill/chip border |

### Typography

```
Display (splash headings):    bold, 28–32px
Section heading (Q titles):   semibold, 22–24px
Body / helper:                regular, 15–16px
Caption / muted label:        regular, 13px
Money amounts (large):        bold, 32–40px, monospaced-figures
```

Font: `DM Sans` for headings and body. `DM Mono` for money amounts. Both via Expo Google Fonts.

### Spacing

Tailwind spacing scale. Base unit `4` (16px). Page horizontal padding: `px-5`. Card padding: `p-4` or `p-5`. Gap between form elements: `gap-4`.

### Radius

Buttons and cards: `rounded-xl` (12px). Pills/chips: `rounded-full`. Inputs: `rounded-lg` (8px).
...
### [SPLASH] Welcome
Heading: "PocketOS" · Subheading: "Your household finances, finally clear." · CTA: "Get started" → Consent
...
```
pocketos_onboarding    → { completed: bool, currentStep: string, percentComplete: number }
pocketos_household     → { type: 'solo'|'partner'|'single_parent', partnerName: string|null, children: [{ displayName: string|null, ageGroup: '0-2'|'3-5'|'6-15'|'16-18' }] }
pocketos_location      → { country: string, city: string|null, currency: string }
pocketos_occupation    → { user: string, partner: string|null }
pocketos_income        → { user: { monthly: number, frequency: string }|null, partner: { monthly: number, frequency: string }|null, otherSources: [{ amount: number, frequency: string, label: string|null }], savingsBalance: number|null, savingsTarget: number|null, goal: { amount: number, targetDate: string, description: string|null }|null }
pocketos_costs         → [{ id: string, amount: number, frequency: string, category: string, subcategory: string|null, dueDate: string|null, renewalDate: string|null, notes: string|null, memberRef: string|null, meta: {} }]
pocketos_debts         → [{ id: string, type: string, balance: number, minPayment: number, apr: number, promoEndDate: string|null, contractEndDate: string|null, paymentDueDay: number|null, notes: string|null }]
pocketos_budget        → { monthlyFlexible: number, rolloverStrategy: 'free'|'capped'|'reset', rolloverMultiplier: number|null, rolloverBalance: number }
pocketos_daily_log     → [{ date: string, spent: number }]
pocketos_alerts        → [{ id: string, type: string, message: string, urgency: 'low'|'medium'|'high', relatedId: string|null, status: 'active'|'snoozed'|'dismissed', snoozedUntil: string|null }]
pocketos_settings      → { currency: string, theme: 'light'|'dark', language: 'en'|'cs', alertLeadDays: number }
```
...
1. **Launch routing:** On app load, check `pocketos_onboarding` in localStorage/AsyncStorage. `completed` false or missing → Wizard welcome. `completed` true → Dashboard.
2. **i18n foundation:** `lib/i18n.js` with `t('key')` helper and two JSON string files (`en.json`, `cs.json`). Populate only keys needed for Increment 1. Language toggle in header persists to `pocketos_settings`.
...
5. **Design tokens:** All colour tokens in `tailwind.config.js` and a `:root` CSS block for web with a `dark` variant.
